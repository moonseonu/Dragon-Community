...
DWORD WINAPI WorkerThread(LPVOID lpParam) {
    ...
    while (TRUE) {
        ...
        if (ioData->operation == 0) { // Receive
            ...
            memcpy(&recvPack.IDSize, ioData->buffer + offset, sizeof(int));
            offset += sizeof(int);
            recvPack.ID = new char[recvPack.IDSize + 1];

            memcpy(recvPack.ID, ioData->buffer + offset, recvPack.IDSize);
            recvPack.ID[recvPack.IDSize] = '\0';
            offset += recvPack.IDSize;

            memcpy(&recvPack.PWSize, ioData->buffer + offset, sizeof(int));
            offset += sizeof(int);
            recvPack.PW = new char[recvPack.PWSize + 1];

            memcpy(recvPack.PW, ioData->buffer + offset, recvPack.PWSize);
            recvPack.PW[recvPack.PWSize] = '\0';
            offset += recvPack.PWSize;

            memcpy(&recvPack.IPSize, ioData->buffer + offset, sizeof(int));
            offset += sizeof(int);
            recvPack.IP = new char[recvPack.IPSize + 1];

            memcpy(recvPack.IP, ioData->buffer + offset, recvPack.IPSize);
            recvPack.IP[recvPack.IPSize] = '\0';
            offset += recvPack.IPSize;

            memcpy(&recvPack.Port, ioData->buffer + offset, sizeof(int));
            offset = 0;
